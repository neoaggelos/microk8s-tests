...
while ! juju run --machine 0 "sudo microk8s.kubectl wait -n mayastor ds/mayastor --for=jsonpath='{.status.numberReady}'=1"
do
...
while ! juju run --machine 0 "sudo microk8s.kubectl get -n mayastor msp | grep Online | wc -l | grep 1"
do
  echo 'waiting for mayastor pool to come online'
  sleep 5
done
...
# wait mayastor to come up
while ! juju run --machine 0 "sudo microk8s.kubectl wait -n mayastor ds/mayastor --for=jsonpath='{.status.numberReady}'=3"
do
  echo 'waiting for mayastor'
  sleep 5
done
...
# wait for mayastor pools to come up
while ! juju run --machine 0 "sudo microk8s.kubectl get -n mayastor msp | grep Online | wc -l | grep 3"
do
